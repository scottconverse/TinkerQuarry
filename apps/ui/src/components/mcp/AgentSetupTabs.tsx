...
interface AgentSetupTabsProps {
  port: number;
  /**
   * Per-boot bearer token the server requires. Without it every snippet below is a config that
   * answers 401 — the card used to describe the header requirement while handing out configs
   * that omitted it. `null` when the server is not running: the snippets then carry a visible
   * placeholder instead of silently working-looking, header-less config.
   */
    ...
    () => [
      {
        id: "claude",
        label: "Claude Code",
        command: buildClaudeMcpCommand(port, sessionToken),
        codeLabel: "Shell",
        instruction:
      ...
      },
      {
        id: "cursor",
        label: "Cursor",
        command: buildCursorMcpConfig(port, sessionToken),
        codeLabel: "JSON",
        locationLabel: "~/.cursor/mcp.json",
        instruction:
          "Add this config in Cursor MCP settings or in ~/.cursor/mcp.json.",
      },
      {
        id: "codex",
        label: "Codex",
        command: buildCodexMcpCommand(port, sessionToken),
        codeLabel: "Shell",
        instruction:
          "Run this command in your terminal to add the TinkerQuarry MCP endpoint.",
      },
      {
        id: "opencode",
        label: "OpenCode",
        command: buildOpenCodeMcpConfig(port, sessionToken),
        codeLabel: "JSON",
        ...
        <TabsContent key={item.id} value={item.id} className="outline-none">
          ...
          >
            ...
            <SetupCodeBlock
              ...
              compact={compact}
              onCopy={() =>
                void copyText(`${item.label} MCP setup`, item.command)
              }
            />
          </div>
        </TabsContent>
