...
`;
  ...
  it("parses the engine template emit_scad output into sliders (TinkerQuarry §6.6)", () => {
    // CORRECTED (E2E-A, 2026-07-21). This fixture used to put the parameters FIRST and
    // `use <library/...>` after, and claimed to be "exactly what emit_scad now produces". It is
    // not: templates.py:emit_scad joins (library, sliders, call) — the `use` line comes FIRST.
    // The order here made the test pass while the shipped feature was broken, which is why
    // nobody knew the Customizer showed no sliders for engine parts at all.
    //
    // This case now pins the PRE-inline text in its real order. The post-inline text — what the
    // UI actually receives, after openscad_runner.inline_library_includes replaces that `use`
    // line with the library's module bodies — is covered in
    // src/utils/customizer/__tests__/inlinedLibraryParams.test.ts.
    const code = `use <library/containers.scad>;
width = 80; // [10:1:170]
depth = 60; // [10:1:170]
wall = 2; // [0.8:0.2:8]
