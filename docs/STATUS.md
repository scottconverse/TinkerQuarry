...
| Accessibility (keyboard/focus/contrast/SR) (§10/§12) | **partial (4 surfaces a11y-tested; 2 real fixes)** | **a11y harness added (2026-06-22):** jest-axe asserting **no serious/critical violations** across four high-traffic surfaces — **describe (WelcomeScreen), export dialog, Customizer slider (ParameterControl), refine panel (AiPromptPanel)**. It caught + I **fixed two real critical** violations: the photo/sketch file input (no name) and the export format combobox (no name) — both `aria-label`. Customizer slider + refine panel verified **clean**. Remaining: extend to the full workspace shell + manual keyboard/focus/contrast/SR passes (jsdom can't compute color-contrast). |
