...
- `prd/TinkerQuarry-PRD-v0.3.md:504` superseded-note: "engine ~1,554 passing of ~1,667 collected (the handful of remaining failures…)".
...
**Evidence:** `prd/TinkerQuarry-PRD-v0.3.md:32-33` "TinkerQuarry generates the geometry, *looks at what it built and fixes it*, checks it …, slices it, and sends it"; `:38` one-liner "Watch it get built, checked, **and corrected**." The loop is **not live** (STATUS:118-120 "being wired into the codegen path … not a present-tense capability"; PRD §13 Table C l.493 agrees).
