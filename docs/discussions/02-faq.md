...
No. You describe the *thing*; you don't draw it. You can fine-tune sizes afterward with sliders.

### Does it use the internet / the cloud?
No, by default. The AI and all the manufacturing run locally. There's an *optional* cloud model you
...
Python 3.13, OpenSCAD, OrcaSlicer, and a local AI (Ollama + a model). The **first-run wizard sets up
the AI for you** — one click, no manual install. Full steps:
...
Slicing uses OrcaSlicer profiles, so anything OrcaSlicer supports. For *direct send*: OctoPrint,
...
### Is it finished?
It's in active development but feature-complete on the software side: the full describe→slice
pipeline works and passes the project's adversarial release gate. See
