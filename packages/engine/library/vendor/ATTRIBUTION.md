...
by first-party `library/threads.scad`, a GPL-2.0 wrapper over vendored BOSL2's BSD-2-Clause thread
modules.
...
- `gridfinity-rebuilt-openscad/src/core/base.scad`: removed the upstream `use
  ...
  thread dependency. Gridfinity's upstream thumbscrew API has not yet been ported to the first-party
  BOSL2-backed thread wrapper.
