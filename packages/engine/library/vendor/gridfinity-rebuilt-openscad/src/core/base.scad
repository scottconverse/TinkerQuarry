...
// TinkerQuarry vendor note:
// The upstream repo includes ../external/threads-scad/threads.scad for the optional
// thumbscrew base feature. That dependency is GPL-3.0-or-later, while TinkerQuarry is
// GPL-2.0-only, so it is intentionally excluded from this vendored copy. Keep
// thumbscrew=false until a clean-room thread library is wired in.
...
module gridfinityBase(grid_size, grid_dimensions=GRID_DIMENSIONS_MM, hole_options=bundle_hole_options(), only_corners=false, thumbscrew=false) {
    ...
    assert(!thumbscrew, "TinkerQuarry's vendored Gridfinity excludes GPL-3.0-or-later threads-scad; thumbscrew requires a clean-room thread library.");
